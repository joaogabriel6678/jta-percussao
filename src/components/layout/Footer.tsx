...
export function Footer() {
  return (
    <footer className="bg-blue-900 text-white pt-16 pb-0" aria-labelledby="footer-title">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="grid md:grid-cols-3 gap-10 mb-12">
          ...
          <nav aria-label="Links do rodapé">
            <h3 className="font-bold text-xs uppercase tracking-widest text-blue-300 mb-5">
              Navegação
            </h3>
            <ul className="space-y-3">
              {NAV_ITEMS.map((item) => (
                <li key={item.href}>
                  ...
                  >
                    <span className="w-1 h-1 rounded-full bg-green-500 inline-block flex-shrink-0" aria-hidden="true" />
                    {item.label}
                  </a>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-3">
              {[
                { icon: "💬", val: SITE_CONFIG.whatsapp, href: WHATSAPP_BASE, extLabel: "WhatsApp" },
                { icon: "✉️", val: SITE_CONFIG.email, href: `mailto:${SITE_CONFIG.email}`, extLabel: "E-mail" },
                { icon: "📍", val: SITE_CONFIG.location, href: undefined, extLabel: "Endereço" },
              ].map((c) => (
                <li key={c.val} className="flex items-center gap-3 text-blue-200 text-sm">
                  ...
                  {c.href ? (
                    <a
                      ...
                      aria-label={c.href.startsWith("http") ? `${c.extLabel} (abre em nova aba)` : c.extLabel}
                    >
                      {c.val}
                    </a>
                  ) : (
      ...
      <div className="bg-gray-950 border-t-2 border-yellow-400/40 py-4 px-4">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-5">
          ...
          <a
            ...
            aria-label="GitHub do desenvolvedor (abre em nova aba)"
          >
          ...
          <a
            href="https://www.linkedin.com/in/jo%C3%A3o-gabriel-xavier-teles-969281369/"
            target="_blank"
            rel="noopener noreferrer"
            ...
            aria-label="LinkedIn do desenvolvedor (abre em nova aba)"
          >
            <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
