      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 grid md:grid-cols-2 gap-14 items-start">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="relative">
            <div
              className="rounded-2xl overflow-hidden bg-cover bg-center w-full"
              style={{
                backgroundImage: "url(/images/hero.png)",
                height: "clamp(220px, 40vw, 420px)",
              }}
              role="img"
              aria-label="JTA Percussão — atabaques artesanais"
            />
          ...
          {/* Quote — fora da imagem, sem sobreposição */}
          <div className="relative bg-white rounded-xl p-5 shadow-md border border-gray-100">
            <div className="absolute -left-1 top-4 w-1 h-8 bg-green-500 rounded-full" />
            <p className="text-gray-700 text-sm italic leading-relaxed">
              &ldquo;Cada instrumento que produzimos carrega a alma da nossa tradição e o
              compromisso com a excelência.&rdquo;
            </p>
            <p className="mt-2 text-blue-700 font-semibold text-sm">— Equipe JTA Percussão</p>
          </div>
        </div>
        ...
        <div>
          ...
          <h2 className="text-3xl md:text-4xl font-extrabold text-blue-800 mb-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {ABOUT_FEATURES.map((f) => (
              <div key={f.title} className="flex gap-3 items-start">
                <span className="text-green-500 text-xl mt-0.5 flex-shrink-0">{f.icon}</span>
                <div>
